...
const vec3 YELLOW  = vec3(186.0, 203.0, 0.0) / 255.0;
...
vec3 background_color(vec2 uv) {
  ...
  float m = uv.y - y;
  col += mix(YELLOW, BLACK, smoothstep(0.0, 1.0, abs(m)));
  col += mix(YELLOW, BLACK, smoothstep(0.0, 1.0, abs(m - 0.8)));
  return col * 0.5;
}
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    if (linesGradient && linesGradient.length > 0) {
      ...
      (uniforms.lineGradientCount.value as number) = stops.length;
      stops.forEach((hex, i) => {
    ...
    const renderLoop = () => {
      ...
      if (interactive) {
        ...
        (uniforms.bendInfluence.value as number) = currentInfluenceRef.current;
      }
      if (parallax) {
